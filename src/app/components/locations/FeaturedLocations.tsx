...
const FeaturedLocations = ({ locations }: FeaturedLocationsProps) => {
  ...
  const finishDragging = (
    ...
    if (!cancelled && finalProgress >= DRAG_COMMIT_PROGRESS) {
  ...
  //==== Chọn địa điểm: phân biệt thao tác click với thao tác kéo thẻ ====
  const selectCard = (event: MouseEvent<HTMLElement>, index: number) => {
        ...
        return (
          ...
          >
            {active ? (
              ...
              <button
                aria-label={`Mở địa điểm ${location.tenViTri}`}
                aria-pressed="false"
            ...
            <div
              className={`absolute inset-0 transition-colors duration-700 ${
                active
                  ? "bg-gradient-to-t from-gray-950/90 via-gray-950/10 to-transparent"
                  : "bg-gray-950/45"
              }`}
            />
            {!active && !dragTarget && (
